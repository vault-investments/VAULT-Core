...
The VAULT Core project operates an open contributor model where anyone is
...
The following applies to code changes to the VAULT Core project, and is not to be
confused with overall VAULT Network Protocol consensus changes.

Whether a pull request is merged into VAULT Core rests with the project merge
...
Patches that change VAULT consensus rules are considerably more involved than
...
Where a patch set proposes to change the VAULT consensus, it must have been
...
The project leader is the release manager for each VAULT Core release.
