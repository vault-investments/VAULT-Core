...
*Setup instructions for a gitian build of VAULT using a Debian VM or physical system.*
...
Clone the git repositories for vault and gitian and then checkout the vault version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vault repository
...
To build VAULT (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vault repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vault/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/vault-investments/VAULT-Core
...
For example:
```bash
URL=https://github.com/vault-investments/VAULT-Core.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit vault=${COMMIT} --url vault=${URL} ../VAULT/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vault=${COMMIT} --url vault=${URL} ../VAULT/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vault=${COMMIT} --url vault=${URL} ../VAULT/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vault-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vault-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vault-build.assert
```
...
[vault-investments/gitian.sigs](https://github.com/vault-investments/gitian.sigs/) repository, or if that's not possible to create a pull
request.
