...
VAULT now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<vault_address\> \<monthly_payment_vault\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ VAULT, _12_ times over the course of a year totaling _24000_ VAULT.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<vault_address\> \<monthly_payment_vault\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` VAULT.
 ...
 * vote-many          - Vote on a VAULT initiative
 * vote-alias         - Vote on a VAULT initiative
 * vote               - Vote on a VAULT initiative/budget
