...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVAULTAmount"))
        settings.setValue("nAnonymizeVAULTAmount", 1000);

    nAnonymizeVAULTAmount = settings.value("nAnonymizeVAULTAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVAULTAmount"))
        SoftSetArg("-anonymizevaultamount", settings.value("nAnonymizeVAULTAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVAULTAmount:
            return QVariant(nAnonymizeVAULTAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVAULTAmount:
            nAnonymizeVAULTAmount = value.toInt();
            settings.setValue("nAnonymizeVAULTAmount", nAnonymizeVAULTAmount);
            emit anonymizeVAULTAmountChanged(nAnonymizeVAULTAmount);
            break;
        case CoinControlFeatures:
