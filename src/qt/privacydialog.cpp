                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVAULT ought to be enough for anybody." - Bill Gates, 2017
    ui->zVAULTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVAULTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVAULT->setEnabled(false);
        ui->pushButtonMintzVAULT->setToolTip(tr("zVAULT is currently disabled due to maintenance."));

        ui->pushButtonSpendzVAULT->setEnabled(false);
        ui->pushButtonSpendzVAULT->setToolTip(tr("zVAULT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVAULTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVAULT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVAULT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVAULT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVAULT in ") +
...
void PrivacyDialog::on_pushButtonSpendzVAULT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVAULT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVAULT
        sendzVAULT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVAULT
    sendzVAULT();
}

void PrivacyDialog::on_pushButtonZVAULTControl_clicked()
{
    ZVAULTControlDialog* zVAULTControl = new ZVAULTControlDialog(this);
    zVAULTControl->setModel(walletModel);
    zVAULTControl->exec();
}

void PrivacyDialog::setZVAULTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVAULTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVAULT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid VAULT Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVAULTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVAULT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VAULT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVAULT</b>";
    ...
    // use mints from zVAULT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVAULTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVAULTControlDialog::GetSelectedMints();
    }
    ...
    // Clear zvault selector in case it was used
    ZVAULTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVAULT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " VAULT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVAULT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVAULT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVAULT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVAULT "));
    ui->labelzVAULTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    // Display AutoMint status

    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zVAULT </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zVAULT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzVAULTSyncStatus->setVisible(fShow);
}
