...
#include "ui_zvaultcontroldialog.h"
...
std::list<std::string> ZVAULTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVAULTControlDialog::listMints;

ZVAULTControlDialog::ZVAULTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVAULTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVAULTControlDialog::~ZVAULTControlDialog()
{
    delete ui;
}

void ZVAULTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVAULTControlDialog::updateList()
{
...
void ZVAULTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVAULTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVAULT_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVAULTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVAULTControlDialog::GetSelectedMints()
{
...
void ZVAULTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
